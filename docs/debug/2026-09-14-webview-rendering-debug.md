...
### Checkpoint 3 target acceptance closure and downstream viewport-unit finding

The target Vivo manually accepted compat `0.1.1`: `repair=measured-layout-px` changed root geometry from `360x0` to `360x670` and the terminal handshake changed from `presentation-degraded` to `presentation-ready`; the DSH home surface visibly rendered. The original blank-home/root-collapse issue is therefore fixed on the target provider.

The newly visible horizontal-sliver failure on Settings/other overlays is downstream and independently attributable to the same WebView viewport-unit bug. Exact-source audit found 17 vertical viewport-unit occurrences in the active official DSH composition. In particular, Settings uses `height:min(800px,100vh - 48px)`, so a provider returning `100vh=0` collapses that internal panel even though `#root` is now healthy. The follow-up remains inside `dsh-webview-compat`: measured-mode-only CSSOM translation of vertical viewport units, with normal engines untouched and the responsive/mobile UI plugin still dormant.

### CP3b vertical viewport CSS compatibility candidate

Compat `0.1.2` extends the already accepted measured root fallback to same-origin CSSOM declarations only while `repair=measured-layout-px`. Exact-source audit showed 17 active official DSH vertical viewport-unit uses, including the Settings panel `height:min(800px,100vh - 48px)` that directly explains the target horizontal-slit screenshot when `100vh=0`. The implementation translates `vh/svh/lvh` from measured layout height and `dvh` from positive visual-viewport height (layout fallback), tracks ownership for cleanup, supports nested/adopted stylesheets, and uses mutation-triggered plus finite startup rescans rather than permanent polling.

Automated gates are green: targeted bridge/parser, full Unit, Debug Build, Lint, Mobile Context Contract, Runtime Alpine E2E including the enhanced deterministic viewport policy, APK embedded-asset SHA equality, static/diff review, and a fresh 360x740 exact-profile Chromium composition sanity. In Chromium, Settings was `312x692`, Directory Picker `312x500`, Models/Plugins remained visible, combo `rev=8990e18694b0` contained current compat and no dormant mobile UI plugin, and console was clean. These Chromium results are non-regression evidence only. Target Vivo acceptance for compat `0.1.2` is still pending and must prove Settings/Models/Plugins/Directory Picker no longer collapse while logs remain `root=360x670`, `presentation-ready`, and report a positive `cssViewportPatches` count.
