...
- **手机 2026-09-17 实际检查更新**：`~/.ssh/config`、`config.bak`、`orange`、`orange.pub` 均不存在；`~/.ssh/known_hosts` mode 600 且 `ssh-keygen -F 40.81.17.167` 找到已保存云服务器主机记录；`~/.ssh/orangepi_recovery_ed25519` 及 `.pub` 存在。名称不证明这个 recovery key 已被 Azure `wechat` 或橙派 `orangepi` 授权。2026-09-08 另有手机 `~/.ssh/dhsp_key.pem` 登录 Azure 成功的历史记录，但本次搜索只覆盖 `~/.ssh`，尚未检查这个 `.ssh` 文件（此前按 `orange*`、`id_ed25519*` 和 `*.pem` 检索结果未显示它，因此可能也已不存在）。下一步：仅在手机本地检查 recovery 公钥指纹、`~/.ssh/dhsp_key.pem` 是否存在；用已存在的 Azure known_hosts 进行 StrictHostKeyChecking=yes、BatchMode=yes 的一次性认证测试，成功后再测试云内 127.0.0.1:22022 转发并恢复两个 Host 配置。绝不重建现有密钥或把私钥复制到对话、Repo 或另一服务器。
