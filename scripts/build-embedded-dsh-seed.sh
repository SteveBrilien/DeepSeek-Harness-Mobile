...
EXPECTED_DSH=$(prop dshVersion)
EXPECTED_PNPM=$(prop pnpmVersion)
EXPECTED_PLUGIN=$(prop mobileContextVersion)
OUT="$ROOT_DIR/core/runtime-android/src/main/assets/$(prop dshSeedAsset)"
PROFILE_OUT="$ROOT_DIR/core/runtime-android/src/main/assets/$(prop webProfileSeedAsset)"
...
python3 - "$E2E_ROOT/dsh-home/profiles/web/package.json" <<'PYPROFILE'
import json,sys
d=json.load(open(sys.argv[1]))
assert '@dsh-mobile/dsh-mobile-context' in d['dsh']['profile']['bundles']
assert 'dsh-client-ui-mobile' not in d.get('dependencies', {})
assert 'dsh-client-ui-mobile' not in d['dsh']['profile']['bundles']
print('native-profile-seed-contract-ok')
PYPROFILE
