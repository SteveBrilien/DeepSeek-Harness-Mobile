		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const remember = (node) => {
					...
					snapshots.set(node, ["height", "min-height", "max-height"].map((property) => ({
						property,
				...
				const restore = (node) => {
					...
					for (const entry of snapshot) {
						...
						if (node.style.getPropertyValue(entry.property) !== owned || node.style.getPropertyPriority(entry.property) !== "important") continue;
				...
				const onChange = () => applyViewportContract();
				mql.addEventListener("change", onChange);
				const observer = new MutationObserver(() => {
					if (!mql.matches) return;
					const root = document.getElementById("root");
					if (root && !snapshots.has(root)) applyViewportContract();
				});
				observer.observe(document.body, { childList: true });
				return () => {
					observer.disconnect();
					mql.removeEventListener("change", onChange);
					for (const node of snapshots.keys()) restore(node);
				};
			}, "ui-mobile: viewport root contract");
			const toggleSidebar = () => {
